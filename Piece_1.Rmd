...
In this piece, I will demonstrate my graphing abilities along with understanding of lmer function.
...
Several different variables were used for the following analyses. Alongside Family Decision Making (FDM), Age (AgeCM), Socio-economic Status (SES), Gender (ChGender), and Country of origin were used to help identify potential trends in the data. Below are the countries utilized for this dataset.
...
My first step was to create a multi-level model. The model below uses Family Decision Making as the outcome. The inputs were gender, country, and an interaction between age and country.

```{r multilevel model}
model.FDM_outcome <- lmer(FDM ~  SES + ChGender + AgeCM + Country + AgeCM*Country + (1|ID), data = data)
FDM_No_Country <- lmer(FDM ~  SES + ChGender + AgeCM + (1|ID), data = data)
```
...
ggplot(
  ...
  geom_line(stat = "summary", fun = "mean", alpha = .5) +
  theme_bw() +
...
data %>%
ggplot(data, aes(x=Age_Exact, y = FDM)) + 
